...
class TestMinusOneVNE(unittest.TestCase):
    ...
    def test_minusOneVNE(self, col_to_remove = 1, num_cpu = 1):
        ...
        InputA = MinusOneVNE(A, ["col1", "col2", "col3", "col4"], num_cpu = 1)
    ...
    def test_compareToFullMatrixVNE(self):
        print('\ntest_MinusOneVNE.compareToFullMatrixVNE:')
        A = numpy.array([[1, 4, 5, 12], [5, 8, 9, 0], [6, 7, 11, 19]])
        InputA = MinusOneVNE(A, ["col1", "col2", "col3", "col4"], num_cpu = 1)
        InputA.compareToFullMatrixVNE()
        my_result = list(numpy.round(numpy.array(InputA.result_summary['delta_abs_vNE']), decimals = 6))

        expected_result = [0.065629, 0.138296, 0.189604, 0.399415]
        self.assertListEqual(my_result, expected_result)
        print('===========================================================')


class TestMinDataLostFilter(unittest.TestCase):

    def test_minDataLostFilter(self):
        print('\ntest_MinDataLostFilter.minDataLostFilter:')
        print('        case 1: test filter result')
        file_name = 'emmer/data/test_case_3.csv'
        input_matrix = RawDataImport(file_name = file_name, for_merging_file = True, suppress = False, second_chance = False)
        input_matrix.readCSV()
        input_matrix.relativeAbundance()
        filtered_data = MinDataLostFilter(input_data = input_matrix.data,
                                          feature_names = input_matrix.feature_names,
                                          sample_id = input_matrix.sample_id,
                                          num_cpu = 1)
        my_result = list(filtered_data.feature_no_to_remove)

        expected_result = [3, 12, 14, 25, 28, 29, 45, 54, 63, 66, 89, 93]
        self.assertListEqual(my_result, expected_result)
        print('===========================================================')
...
class TestInfoRichCalling(unittest.TestCase):
    ...
    def test_infoRichSelect(self):
        ...
        with self.assertRaises(ErrorCode4):
            ...
            info_rich_result = InfoRichCalling(data = filtered_data.data, current_feature_names = filtered_data.feature_names,
                                               upper_threshold_factor = 100, lower_threshold_factor = 100,
...
class Test_reproducibility_summary(unittest.TestCase):
    ...
    def test_reproducibility_summary(self):
        ...
        expected_result = [4, 2, 3, 1, 2]
        self.assertListEqual(my_result, expected_result)

        my_result = list(info_rich_features_w_reproducibility['feature_name'])
        expected_result = ['col3', 'col5', 'col2', 'col1', 'col6']
        self.assertListEqual(my_result, expected_result)
...
class TestKernal(unittest.TestCase):
    ...
    def test_importAndProcess(self):
        ## use "MinDataLostFilter"
        print('\ntest_Kernal.importAndProcess:')
        print('        case 1: MinDataLostFilter')
        print('             1.1: set detection limit correctly?')
        print('    file_names: test_case_3.csv')
        file_names = 'emmer/data/test_case_3.csv'
        print('    detection_limit: 0')
        detection_limit = 0
        print('    tolerance: 1')
        tolerance = 1
        print('    filter: "MinDataLostFilter"')
        filter = 'MinDataLostFilter'
        print('    upper_lim: 1')
        upper_lim = 1
        print('    lower_lim: 1')
        lower_lim = 1
        ...
        data = Kernal(file_name = file_names, detection_limit = detection_limit, tolerance = tolerance,
                      ...
                      notebook_name = '', num_cpu = num_cpu, neglect = True)
        ...
        expected_result = 'MinDataLostFilter'
        self.assertEqual(my_result, expected_result)


        print('        ---------------------------------------------------')
        print('             1.3: remove correct features')
        my_result = list(data.filtered_data.feature_no_to_remove)
        expected_result = [3, 12, 14, 25, 28, 29, 45, 54, 63, 66, 89, 93]
        self.assertListEqual(my_result, expected_result)
        ...
        ## use "HardFilter"
        print('        ---------------------------------------------------')
        print('        case 2: HardFilter (only set zero_tolerance)')
        ...
        data = Kernal(file_name = file_names, detection_limit = detection_limit, tolerance = tolerance,
                      ...
                      quick_look = quick_look_1, use_fractional_abundance = use_fractional_abundance,
                      vNE_output_folder = vNE_output_folder, output_file_tag = output_file_tag,
                      num_cpu = num_cpu, notebook_name = '', neglect = True)
        data.importAndProcess()
        ...
        with self.assertRaises(ErrorCode6):
            data = Kernal(file_name = file_names, detection_limit = detection_limit, tolerance = tolerance,
                          ...
                          quick_look = quick_look_1, use_fractional_abundance = use_fractional_abundance,
                          vNE_output_folder = vNE_output_folder, output_file_tag = output_file_tag,
                          num_cpu = num_cpu, neglect = True, notebook_name = '', suppress = True)
            data.importAndProcess()


        ## use "None"
        print('        ---------------------------------------------------')
        print('        case 3: None (only set zero_tolerance)')
        print('             3.1: set detection limit correctly?')
        print('    file_names: test_case_3.csv')
        file_names = 'emmer/data/data_dir_1/test_case_1.csv'
        print('    detection_limit: 0')
        detection_limit = 0
        ...
        print('        ---------------------------------------------------')
        print('             3.2: set filter correctly?')
        my_result = data.filter
        ...
        print('        ---------------------------------------------------')
        print('             3.3: remove correct features')
        my_result = list(data.filtered_data.data.columns.values)
        ...
        print('        ---------------------------------------------------')
        print('        case 4: set detection limits')
        print('             4.1: set detection limits correctly')
        print('    file_names: test_case_3.csv')
        file_names = 'emmer/data/data_dir_1/test_case_1.csv'
        ...
        print('        ---------------------------------------------------')
        print('             4.2: set detection filter correctly')
        my_result = data.filter
        expected_result = 'None'
        self.assertEqual(my_result, expected_result)
    ...
    ## Test information-rich feature calling threshold
    def test_infoRichCallingAndReproducibility(self):
        print('\ntest_Kernal.infoRichCallingAndReproducibility:')
        print('        case 1: set information-rich feature calling threshold')
        print('    file_names: test_case_1.csv')
        file_names = 'emmer/data/test_case_1.csv'
        print('    detection_limit: 0')
        detection_limit = 0
        ...
        print('    lower_lim: 1')
        lower_lim = 0.5
        print('    information-rich threshold: 5')
        infoRich_threshold = 5
        print('    quick_look_2: False')
        quick_look_2 = False
        print('    use_fractional_abundance: True')
        use_fractional_abundance = True
        ...
        data.infoRichCallingAndReproducibility()
        my_result = list(data.info_rich_features_w_reproducibility['feature_name'])
        expected_result = ['col1', 'col2', 'col3', 'col6']
        self.assertListEqual(my_result, expected_result)


        ## Maximum number of information-rich feature
        print('        ---------------------------------------------------')
        print('        case 1: maximum number of information-rich feature')
        print('             1.1 at quick look mode')
        ...
        print('        ---------------------------------------------------')
        print('             1.2 report WarningCode11')
        ...
        print('        ---------------------------------------------------')
        print('             2.2 quick look mode off')
        print('             2.2.1 ability to prevent report more information-rich feature than the maximum cap (Warning code 11, 12)')
        print('    file_names: data/data_dir_4/group_A.csv')
        file_names = 'emmer/data/data_dir_4/group_A.csv'
        print('    detection_limit: 0.001')
        detection_limit = 0.001
        ...
        print('        ---------------------------------------------------')
        print('             2.2.2 report WarningCode12')
        my_result = data.warning_code       
        expected_result = '12'
        self.assertEqual(my_result, expected_result)
