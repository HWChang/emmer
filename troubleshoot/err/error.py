...
class ErrorCode35(Error):
    ...
    def __init__(self, suppress):
        ...
        Hint: the file stored under output/ that contains 'scaler' in file name.
        """
        return(suppress)
...
class ErrorCode42(Error):
    ...
    def __init__(self, suppress):
        ...
        [[Error code 42]]
        Parameter setting error:
        Input 'transformation_matrix' and 'scaler' might be from different analyses. Please
        choose the 'transformation_matrix' file and 'scaler' file generated from the same
        emmer.harvest run

        To pass:
        Make sure both file have exactly the same set of feature names (as rowname in
        'transformation_matrix'; as colname in 'scaler') and they are at the same order
        """
        return(suppress)
